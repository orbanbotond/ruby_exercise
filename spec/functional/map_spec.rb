...
describe 'Map' do
  before do
    create_temporary_class 'WrapperClass' do
  ...
  specify 'simple' do
    expect(WrapperClass.new.map_simple([1,2,3]){|x| 2 * x}).to eq([2,4,6])
  end

  specify 'side effect less map' do
    expect(WrapperClass.new.map_recursive([1,2,3]){|x| 2 * x}).to eq([2,4,6])
  end

  specify 'side effect less map non recursive' do
    expect(WrapperClass.new.map_side_effect_less_non_recursive([1,2,3]){|x| 2 * x}).to eq([2,4,6])
  end

  specify 'recursive map take more time x1.6' do
    b1 = Benchmark.measure do
        1000000.times { WrapperClass.new.map_simple([1,2,3]){|x| 2 * x} }
    end
  ...
  specify 'recursive vs non recursive' do
    b1 = Benchmark.measure do
        1000000.times { WrapperClass.new.map_side_effect_less_non_recursive([1,2,3]){|x| 2 * x} }
    end
    b2 = Benchmark.measure do
        1000000.times { WrapperClass.new.map_recursive([1,2,3]){|x| 2 * x} }
    end
